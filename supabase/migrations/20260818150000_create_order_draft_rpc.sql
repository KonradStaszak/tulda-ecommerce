...
begin
  ...
  into v_order_id, v_order_number, v_order_status, v_payment_status, v_subtotal_minor, v_currency, v_existing_hash
  from public.orders as o
  where o.idempotency_key = v_idempotency_key;

  if found then
    if v_existing_hash <> v_idempotency_hash then
      raise exception using errcode = 'P0001', message = 'IDEMPOTENCY_CONFLICT';
    end if;

    return query select v_order_id, v_order_number, v_order_status, v_payment_status, v_subtotal_minor, v_currency, true;
    return;
  end if;
  ...
  begin
    ...
    into v_order_id, v_order_number, v_order_status, v_payment_status;
  exception when unique_violation then
    ...
    if not found then
      raise;
    elsif v_existing_hash <> v_idempotency_hash then
      raise exception using errcode = 'P0001', message = 'IDEMPOTENCY_CONFLICT';
    end if;
