...
export default { fetch: withSupabase({ auth: 'publishable' }, async (request, ctx) => {
  ...
  const fullName = text(customer.fullName); const customerEmail = text(customer.email).toLowerCase(); const phone = text(customer.phone)
  if (!uuid.test(text(body?.idempotency_key)) || !fullName || !phone || !email.test(customerEmail) || !Array.isArray(lines) || !lines.length) return Response.json({ error: 'Invalid request.' }, { status: 400 })
  ...
  const { data: quote, error: quoteError } = await ctx.supabaseAdmin.from('quote_requests').insert({ idempotency_key: body.idempotency_key, user_id: userData.user?.id ?? null, first_name: fullName, last_name: '', company: text(customer.company) || null, email: customerEmail, phone, notes: text(customer.notes) || null }).select('id,request_number').single()
