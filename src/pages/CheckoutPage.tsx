...
function TextField({ id, label, value, error, required = false, type = 'text', autoComplete, onChange }: TextFieldProps) {
  ...
  return <div>
    ...
    <input id={id} type={type} value={value} autoComplete={autoComplete} required={required} aria-invalid={Boolean(error)} aria-describedby={error ? errorId : undefined} onChange={(event) => onChange(event.target.value)} className="tulda-field mt-2 w-full px-3 py-3 text-sm" style={{ borderColor: error ? 'var(--color-danger)' : undefined }} />
    {error && <p id={errorId} className="mt-1 text-xs" role="alert" style={{ color: 'var(--color-danger)' }}>{error}</p>}
  </div>
}
...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const loadSavedDetails = async () => {
      ...
      const { data: profile } = await customerDb.from('customer_profiles').select('*').eq('user_id', userData.user.id).maybeSingle()
      if (!profile) return
      const nameParts = String(profile.full_name ?? '').trim().split(/\s+/)
      setForm((current) => ({
        ...current,
        customer: { ...current.customer, email: userData.user.email ?? '', firstName: nameParts[0] ?? '', lastName: nameParts.slice(1).join(' '), phone: profile.phone ?? '', company: profile.company ?? '' },
        deliveryAddress: { country: profile.country ?? 'United Kingdom', addressLine1: profile.address_line_1 ?? '', addressLine2: profile.address_line_2 ?? '', city: profile.city ?? '', region: profile.region ?? '', postcode: profile.postcode ?? '' },
      }))
    }
  ...
  if (lines.length === 0) return <main className="max-w-[1100px] mx-auto px-6 py-10"><nav className="text-xs" style={{ color: 'var(--muted-foreground)' }}><Link to="/">Home</Link> / Checkout</nav><section className="mt-8 py-20 text-center border" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--muted)' }}><h1 className="text-3xl font-black" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>YOUR CHECKOUT IS EMPTY</h1><p className="mt-3 text-sm" style={{ color: 'var(--muted-foreground)' }}>Add products to your cart before continuing to checkout.</p><Link to="/products" className="tulda-button mt-6">SHOP PRODUCTS</Link></section></main>

  if (hasUnavailableItems) return <main className="max-w-[1100px] mx-auto px-6 py-10"><nav className="text-xs" style={{ color: 'var(--muted-foreground)' }}><Link to="/">Home</Link> / <Link to="/cart">Cart</Link> / Checkout</nav><section className="mt-8 p-8 border" style={{ borderColor: 'var(--color-danger)', backgroundColor: 'var(--color-danger-soft)' }}><h1 className="text-3xl font-black" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>CHECKOUT IS PAUSED</h1><p className="mt-3 text-sm">One or more cart items are unavailable. Update your cart before continuing to checkout.</p><Link to="/cart" className="tulda-button mt-6">REVIEW CART</Link></section></main>
  ...
  return <main className="max-w-[1400px] mx-auto px-6 py-10">
    ...
    <div className="mt-8 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_400px] gap-10 items-start">
      <form noValidate onSubmit={handleSubmit} className="border p-5 sm:p-7" style={{ borderColor: 'var(--border)', boxShadow: 'var(--shadow-subtle)' }}>
        {Object.keys(errors).length > 0 && <div className="mb-6 border p-4 text-sm" role="alert" style={{ borderColor: 'var(--color-danger)', backgroundColor: 'var(--color-danger-soft)' }}>Please correct the highlighted fields before continuing.</div>}
        ...
        <div className="mt-8"><label htmlFor="order-notes" className="block text-2xl font-bold" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>ORDER NOTES <span className="text-sm font-normal" style={{ fontFamily: 'Inter, sans-serif' }}>(optional)</span></label><textarea id="order-notes" value={form.orderNotes} onChange={(event) => { setForm((current) => ({ ...current, orderNotes: event.target.value })); resetDraftAttempt() }} rows={4} className="tulda-field mt-3 w-full p-3 text-sm" /></div>
        <button type="submit" disabled={isSubmitting} className="tulda-button mt-8 w-full disabled:opacity-60">{isSubmitting ? 'CREATING DRAFT ORDER...' : 'CONTINUE TO PAYMENT'}</button>
        {submissionError && <section className="mt-5 border p-4 text-sm" role="alert" style={{ borderColor: 'var(--color-danger)', backgroundColor: 'var(--color-danger-soft)' }}>{submissionError}</section>}
