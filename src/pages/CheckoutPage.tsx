...
export default function CheckoutPage() {
  ...
  const updateCustomer = (field: keyof CheckoutFormData['customer'], value: string) => setForm((current) => ({ ...current, customer: { ...current.customer, [field]: value } }))
  const updateAddress = (key: 'deliveryAddress' | 'billingAddress', field: keyof Address, value: string) => setForm((current) => ({ ...current, [key]: { ...current[key], [field]: value } }))

  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    const nextErrors = validateCheckoutForm(form)
    setErrors(nextErrors)
    setReadyForPayment(Object.keys(nextErrors).length === 0)
  }
  ...
  return <main className="max-w-[1400px] mx-auto px-6 py-10">
    ...
    <div className="mt-8 grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_400px] gap-10 items-start">
      <form noValidate onSubmit={handleSubmit} className="border p-5 sm:p-7" style={{ borderColor: 'var(--border)' }}>
        ...
        <div className="mt-8 border-t pt-7" style={{ borderColor: 'var(--border)' }}><label className="flex items-start gap-3 text-sm cursor-pointer"><input type="checkbox" checked={form.billingSameAsDelivery} onChange={(event) => { setForm((current) => ({ ...current, billingSameAsDelivery: event.target.checked })); setReadyForPayment(false) }} className="mt-0.5 h-4 w-4 accent-[var(--primary)]" /><span>Billing address is the same as delivery address</span></label>{!form.billingSameAsDelivery && <AddressFields idPrefix="billing" address={form.billingAddress} errors={errors} onChange={(field, value) => updateAddress('billingAddress', field, value)} />}</div>
        <div className="mt-8"><label htmlFor="order-notes" className="block text-2xl font-bold" style={{ fontFamily: 'Barlow Condensed, sans-serif' }}>ORDER NOTES <span className="text-sm font-normal" style={{ fontFamily: 'Inter, sans-serif' }}>(optional)</span></label><textarea id="order-notes" value={form.orderNotes} onChange={(event) => { setForm((current) => ({ ...current, orderNotes: event.target.value })); setReadyForPayment(false) }} rows={4} className="mt-3 w-full border p-3 text-sm bg-white" style={{ borderColor: 'var(--border)' }} /></div>
        <button type="submit" className="mt-8 w-full py-3 text-sm font-semibold rounded-sm" style={{ backgroundColor: 'var(--primary)', color: '#fff' }}>CONTINUE TO PAYMENT</button>
        {readyForPayment && <section className="mt-5 border p-4 text-sm" role="status" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--muted)' }}><strong>Payment integration is the next step.</strong><p className="mt-1">Your details remain in this page only. No order, payment, or cart change has been made.</p></section>}
      </form>
